...
script_body() {
    ...
    #
    iptables -A INPUT -i eth0   -s 10.0.1.0/24   -m state --state NEW  -j DROP
    #
    iptables -A FORWARD -i eth0   -s 198.51.100.2   -m state --state NEW  -j DROP
    #
    iptables -A FORWARD -i eth0   -s 10.0.1.0/24   -m state --state NEW  -j DROP
    ...
    #
    iptables -A FORWARD -i eth0 -p TCP -s 198.51.100.10 -d 10.0.1.10 --dport 25 -m state --state NEW  -j ACCEPT
    #

    # 
    echo "... permit access from LAN to Internet and DMZ"
    #
    iptables -A FORWARD -i eth1 -s 10.0.1.0/24 -d 198.51.100.0/24 -m state --state NEW  -j ACCEPT
    iptables -A FORWARD -i eth1 -s 10.0.1.0/24 -m state --state NEW  -j ACCEPT
    #
}
